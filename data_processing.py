...
def unzip_all(source_dir_path_list, dest_dir_path,
              skip=False):  # source_dir_path_list 안에 있는 각각의 디렉토리 내 zip 파일을 압축 해제하여 dest_dir_path에 저장
    ...
    for source_rdir_path in source_dir_path_list:  # zip 파일을 저장하고 있는 각각의 source 디렉토리에 대해 반복
        ...
        zip_file_name_list = [file_name for file_name in file_name_list if
                              zipfile.is_zipfile(os.path.join(source_rdir_path, file_name))]  # 파일 중 zip 파일만 filter
        ...
        for zip_file_name in zip_file_name_list:  # 각 zip 파일을 압축 해제
            ...
            extract_dest_dir_path = os.path.join(dest_dir_path, source_dir_name,
                                                 zip_file_basename)  # 압축 해제 한 파일들을 저장할 경로

            if os.path.exists(
                    extract_dest_dir_path) and skip == True:  # 이미 압축 해제된 디렉토리가 있고, skip 파라미터가 true인 경우 해당 zip파일을 압축 해제하지 않음.
                continue
...
def save_data(data, pickle_file_path):
    with open(pickle_file_path, "wb") as img_data_pickle:
        pickle.dump(data, img_data_pickle,
                    protocol=pickle.HIGHEST_PROTOCOL)  # The advantage of HIGHEST_PROTOCOL is that files get smaller. This makes unpickling sometimes much faster.
